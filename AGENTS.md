...
- **Ignore columns support**: `getRows` accepts `ignoreColumns` and forwards it to backend. Typing uses `OmitIgnored<...>`: literal ignore lists narrow return data shape; widened key unions keep safe non-narrowed typing.
...
- `DatabaseListOptions`: Comprehensive options for row retrieval (pagination, sorting, filtering, `ignoreColumns`)
...
- `OmitIgnored<T, K>`: Utility type used by `getRows` to omit ignored columns when keys are known literals, with safe fallback for widened key unions
